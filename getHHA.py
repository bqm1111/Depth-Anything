...
def getImage(root='demo'):
    D = cv2.imread(os.path.join(root, '0.png'), cv2.COLOR_BGR2GRAY)
    RD = cv2.imread(os.path.join(root, '0_raw.png'), cv2.COLOR_BGR2GRAY)
    print(np.max(RD))
    return D, RD
...
if __name__ == "__main__":
    ...
    cv2.imshow('demo/hha.png', hha)
    cv2.imshow('demo/hha_complete.png', hha_complete)
    cv2.waitKey()
