...
if __name__ == "__main__":
    ...
    # depth_anything = DepthAnything.from_pretrained("ckpts/depth_anything_vitl14.pth").to(DEVICE).eval()
    ...
    real_depth_path = "/home/sherlock/Pictures/segmentation/NYUDepthv2/Depth"

    for filename in tqdm(filenames):
        raw_image = cv2.imread(filename)
        image_filename = filename.split("/")[-1]
        real_depth_img = cv2.imread(
            os.path.join(real_depth_path, image_filename.split(".")[0] + ".png")
        )
        ...
        # depth = np.concatenate((raw_image, depth, real_depth_img), axis=1)

        if args.pred_only:
            cv2.imwrite(
                os.path.join(
                    args.outdir, filename[: filename.rfind(".")] + ".png"
                ),
                depth,
            )
        else:
